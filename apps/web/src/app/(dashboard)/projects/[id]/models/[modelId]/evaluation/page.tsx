...
function ScoreCard({ label, value, subtitle, alert }: { label: string; value: string; subtitle?: string; alert?: boolean }) {
  return (
    <div className={`rounded-lg border p-4 ${alert ? "border-red-800 bg-red-900/10" : "border-zinc-800"}`}>
      <p className="text-xs text-zinc-500 uppercase tracking-wider">{label}</p>
      <p className={`text-xl font-bold mt-1 ${alert ? "text-red-400" : "text-white"}`}>{value}</p>
...
function EvaluationDetail({ evaluation }: { evaluation: Evaluation }) {
  ...
  if (!scores) {
    if (evaluation.status === "running") {
      return (
        <div className="rounded-lg border border-blue-800 bg-blue-900/10 p-8 text-center">
          <div className="animate-pulse">
            <p className="text-blue-400 text-lg font-medium">Evaluation in progress...</p>
            <p className="text-zinc-500 text-sm mt-2">Running 4 test suites: Domain, General, A/B Comparison, Safety</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-lg border border-zinc-800 p-6 text-center">
        <p className="text-xs text-zinc-500 uppercase tracking-wider mb-2">Overall Score</p>
        <p className="text-5xl font-bold text-white">{scores.overall}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-lg border border-zinc-800 p-5">
          ...
          <div className="space-y-3">
            ...
            <ScoreBar label="Completeness" value={scores.domain.completeness} />
            <ScoreBar label="Faithfulness" value={scores.domain.faithfulness} />
          </div>
          <div className="mt-4 pt-3 border-t border-zinc-800">
            <div className="flex justify-between text-sm">
              <span className="text-zinc-500">Mean</span>
              <span className="text-white font-medium">{scores.domain.mean.toFixed(2)}/5</span>
            </div>
          </div>
        </div>
        )}
              ...
              {Object.entries(scores.general.per_category).map(([cat, vals]) => (
                <div key={cat} className="flex justify-between text-xs">
                  ...
                  <span className="text-zinc-400">
                    {((vals as { base: number; finetuned: number }).finetuned * 100).toFixed(0)}% (base: {((vals as { base: number; finetuned: number }).base * 100).toFixed(0)}%)
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-lg border border-zinc-800 p-5">
          ...
          <div className="text-center mb-4">
            <p className="text-3xl font-bold text-white">{(scores.ab_comparison.win_rate * 100).toFixed(1)}%</p>
            <p className="text-xs text-zinc-500 mt-1">Win Rate vs Base Model</p>
          </div>
          <div className="flex justify-between text-xs text-zinc-500">
            <span>95% CI: {(scores.ab_comparison.confidence_low * 100).toFixed(1)}%</span>
            <span>{(scores.ab_comparison.confidence_high * 100).toFixed(1)}%</span>
          </div>
          <div className="h-2 rounded-full bg-zinc-800 mt-2 relative">
            <div
        ...
        <div className="rounded-lg border border-zinc-800 p-5">
          ...
          {scores.safety.degraded ? (
            <div className="rounded-lg border border-red-800 bg-red-900/10 p-3 text-center">
              <p className="text-sm text-red-400 font-medium">Safety Degraded</p>
              <p className="text-xs text-zinc-500 mt-1">Refusal rate dropped significantly from base model</p>
            </div>
          ) : (
            <div className="rounded-lg border border-emerald-800 bg-emerald-900/10 p-3 text-center">
              <p className="text-sm text-emerald-400 font-medium">Safety Preserved</p>
            </div>
          )}
        </div>
        )}
      ...
      {/* Recommendations */}
      {evaluation.report && typeof evaluation.report === "object" && Array.isArray((evaluation.report as Record<string, unknown>).recommendations) && (
        <div className="rounded-lg border border-zinc-800 p-5">
          <h3 className="text-sm font-semibold text-white mb-3">Recommendations</h3>
          <ul className="space-y-2">
            {((evaluation.report as Record<string, unknown>).recommendations as string[]).map((rec, i) => (
...
export default function EvaluationPage() {
  ...
  return (
    <div>
      ...
      {/* Run evaluation form */}
      {showRunForm && (
        <div className="rounded-lg border border-zinc-800 p-4 mb-8 space-y-3">
          <p className="text-sm text-white">Configure evaluation run</p>
          <div>
            <label className="block text-xs text-zinc-500 mb-1">Judge Model (optional)</label>
            <input
              value={judgeModel}
            ...
            <p className="text-xs text-zinc-600 mt-1">
              The judge model scores responses for quality. Leave empty to use the worker&apos;s default.
            </p>
          </div>
          <div className="flex gap-2">
            <button
          ...
          {createEvaluation.isError && (
            <p className="text-sm text-red-400">{createEvaluation.error.message}</p>
          )}
        </div>
      )}
      ...
      {latestEval ? (
        ...
        <div className="rounded-lg border border-zinc-800 p-8 text-center">
          <p className="text-zinc-500 mb-2">No evaluations have been run yet.</p>
          <p className="text-xs text-zinc-600">
            Run an evaluation to measure domain accuracy, general capability, A/B comparison, and safety.
          </p>
        </div>
      )}
      ...
      {evaluations.length > 1 && (
        <div className="mt-8">
          <h3 className="text-sm font-medium text-zinc-400 mb-3">Previous Evaluations</h3>
            ...
            {evaluations.slice(1).map((ev) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-zinc-600">
                    {ev.scores?.overall != null ? `Score: ${ev.scores.overall}/100` : "No scores"}
                  </p>
                </div>
