...
/// Shown when an explicit teacher contradicts the one that wrote the dataset.
pub const TEACHER_MISMATCH_MESSAGE: &str = "This teacher is not the one that generated the dataset. A distill job inherits \
     its teacher from the data, so leave it out or name the same model.";
