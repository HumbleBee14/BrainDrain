...
mod tests {
    ...
    fn empty_policy_is_disabled_and_never_blocks() {
        let policy = DeployGatePolicy::from_thresholds(None, None);
    ...
    fn win_rate_at_or_above_threshold_passes() {
        let policy = DeployGatePolicy::from_thresholds(Some(0.5), None);
        assert!(matches!(
    ...
    fn win_rate_below_threshold_blocks() {
        let policy = DeployGatePolicy::from_thresholds(Some(0.6), None);
    ...
    fn regression_within_budget_passes() {
        ...
        let policy = DeployGatePolicy::from_thresholds(None, Some(10.0));
        assert!(matches!(
    ...
    fn both_rules_can_fail_together() {
        let policy = DeployGatePolicy::from_thresholds(Some(0.6), Some(10.0));
