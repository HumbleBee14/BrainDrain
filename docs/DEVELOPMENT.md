...
| `TEACHER_GPU_SPEND_CAP_STARTER` / `_GROWTH` / `_PRO` | API | Monthly USD ceiling, per plan tier, on teacher-scoring GPU time (high-fidelity distillation runs the teacher on our own GPUs). Checked once at job admission: the estimate plus this month's **delivered** `extraction` billing rows must not exceed the cap. Covers on-policy improve passes as well as extraction — both spend our own metered GPU running somebody's teacher. Because it counts delivered rows, runs admitted close together can jointly exceed it, and a run already scoring is never stopped mid-way — the same semantics as every other spend cap here. | Uncapped for that tier; admission never refuses on cost |
