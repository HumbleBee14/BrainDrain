...
| `TEACHER_GPU_SPEND_CAP_STARTER` / `_GROWTH` / `_PRO` | API | Monthly USD ceiling, per plan tier, on teacher-scoring GPU time (high-fidelity distillation runs the teacher on our own GPUs). Checked once at job admission: the estimate plus this month's **delivered** `extraction` and `teacher_serving` billing rows must not exceed the cap. Both spend our own metered GPU running somebody's teacher — extraction as its own job, `teacher_serving` as the teacher's device-count share of an on-policy training container, which that container writes as a second row alongside its `training` row. Because it counts delivered rows, runs admitted close together can jointly exceed it, and a run already scoring is never stopped mid-way — the same semantics as every other spend cap here. | Uncapped for that tier; admission never refuses on cost |
| `ON_POLICY_TOKENS_PER_SEC` | API | Completion tokens per second assumed when quoting an on-policy improve pass, covering both the student writing an answer and the teacher scoring it. **The one figure in that quote not measured on real hardware** — rollout speed depends on student size, rollout backend and answer length — so the default is pessimistic and should be replaced with a measured value per deployment. | `40` |
| `MODAL_ON_POLICY_FUNCTION_NAME` | Workers | Deployed Modal function for on-policy runs. Separate from `MODAL_FUNCTION_NAME` because the teacher runs as a `trl vllm-serve` sidecar in the trainer's container, which needs vLLM installed — and unsloth cannot resolve beside vLLM, so it is a different image. | `train_on_policy` |
| `HOSTED_TEACHER_CATALOG_PATH` | API | JSON array **replacing** the built-in list of teachers we may run on our own GPUs, so adding a model is a config change rather than a release. Each entry needs `model_id`, a full 40-char `revision` commit sha, `license`, `display_name`, `size`, `gpu_class`, `est_scored_tokens_per_sec`, `student_family[]`. A malformed file fails startup. | Built-in catalog (Qwen3-32B, Qwen2.5-32B-Instruct) |
| `RESTRICTED_TEACHER_HOSTS` | API | Comma-separated hosts whose terms restrict training on their outputs, **replacing** the built-in list. A teacher on one of these needs an explicit acknowledgment before a job starts. Empty value restricts nothing. | Built-in list of known proprietary API hosts |
